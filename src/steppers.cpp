...
void steppersInit()
{
    ...
    if (ttStepper)
    {
        ttStepper->setDirectionPin(TT_DIR_PIN, true);
    ...
    if (thStepper)
    {
        thStepper->setDirectionPin(TH_DIR_PIN, false);
...
void turntableGoToSlot(uint16_t slot)
{
    ...
    logMsg("TT: slot %u -> %u (%.3f deg, %ld steps)",
...
int32_t turntableCurrentSteps()
{
    return ttStepper ? ttStepper->getCurrentPosition() : 0;
}
